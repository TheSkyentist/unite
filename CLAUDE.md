...
**`unite/line/config.py`** — `LineConfiguration`: main container for line specs (emission and absorption). Token classes: `Redshift`, `FWHM`, `LineShape`, `Flux`, `Tau` — shared tokens across lines produce a single model parameter. `Flux` is for emission lines, `Tau` (dimensionless optical depth) is for absorption lines. Whether a line is emission or absorption is determined by the token: pass `tau=Tau()` for absorption, otherwise defaults to `Flux()`. Any profile shape can be used for either emission or absorption. Passing both `flux` and `tau` raises `TypeError`. `ConfigMatrices` includes `tau_names`/`tau_matrix`/`is_tau` alongside the existing flux/redshift/fwhm matrices; `is_tau` is derived from whether each entry has a `Tau` token.
