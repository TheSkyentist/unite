...
**`unite/model.py`** — `ModelBuilder` assembles line/continuum configurations and spectra into a numpyro model. `build()` returns `(model_fn, model_args)` and accepts `absorber_position` (`'foreground'`, `'behind_lines'`, `'behind_continuum'`) to control where absorbing material sits relative to emission/continuum sources, and `integration_mode` (`'analytic'`, `'quadrature'`, or `'convolution'`) to select how line profiles are integrated over pixels. `fit()` is a convenience NUTS wrapper returning `(samples, model_args)`. Handles parameter sharing matrices, priors, topological sorting, and the emission/absorption split (tau is kept separate from flux_scale throughout).
...
**`unite/_compose.py`** — Private module with pure JAX composition functions shared by `model.py` and `evaluate.py`. `compose_from_profiles` combines pre-evaluated emission profiles, absorption profiles, and continuum into a total model according to `absorber_position`. `compose_leave_one_out` additionally computes exact per-line contributions via leave-one-out differencing.
...
**`unite/line/config.py`** — `LineConfiguration`: main container for line specs (emission and absorption). Token classes: `Redshift`, `FWHM`, `LineShape`, `Flux`, `Tau` — shared tokens across lines produce a single model parameter. `Flux` is for emission lines, `Tau` (dimensionless optical depth) is for absorption lines. Whether a line is emission or absorption is determined by the token: pass `tau=Tau()` for absorption, otherwise defaults to `Flux()`. Any profile shape can be used for either emission or absorption. Passing both `flux` and `tau` raises `TypeError`. `ConfigMatrices` includes `tau_names`/`tau_matrix`/`is_absorption` alongside the existing flux/redshift/fwhm matrices; `is_absorption` is derived from whether each entry has a `Tau` token.
...
- **Absorption support** — mention tau-parametrized lines and absorber position whenever documenting the model.
